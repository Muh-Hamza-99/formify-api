...
const restrictToAdmin = catchAsync(async (req, res, next) => {
    if ((req.user.username === process.env.ADMIN_USERNAME) && (req.user.email === process.env.ADMIN_EMAIL) && (await correctPassword(process.env.ADMIN_PASSWORD, req.user.password))) {
